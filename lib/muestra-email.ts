...
export type MuestraEmailResult =
  | { ok: true; to: string; subject: string; html: string; requestNumber: string }
...
export async function buildMuestraEmail(
  ...
  const { data: req } = await supabase
    ...
    .select(
      "request_number, status, reason, notes, training_people, ship_to_client, ship_date, account_id, sales_reps:sales_rep_id(full_name), accounts:account_id(business_name), sample_request_items(product_name, supplier, quantity, notes)",
    )
    .eq("id", sampleId)
    .maybeSingle();
