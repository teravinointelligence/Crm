...
// Aceite de oliva: categoría propia porque las reglas de muestras lo tratan
// distinto (es de un solo uso, ver migración 0101).
const OIL_WORDS = ["aceite de oliva", "olive oil", "aceite oliva", "aove", "aceite"];
...
export function inferCategory(p) {
  ...
  // 1) Aceite de oliva (señal inequívoca; ni siquiera es una bebida).
  let m = hasAny(hay, OIL_WORDS);
  if (m) return { category: "aceite", confidence: lvl(m), reason: `Contiene "${m}" (aceite de oliva)` };

  // 2) Destilados (señal muy fuerte).
  m = hasAny(hay, SPIRIT_WORDS);
  ...
  // 9) Varietal. Si hay tinto y blanco a la vez (ensamble), es ambiguo.
  ...
  // 10) Sin señal: no se puede inferir por reglas.
  return null;
}
