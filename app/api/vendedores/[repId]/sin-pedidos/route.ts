// GET  /api/vendedores/[repId]/sin-pedidos?days=21 → borrador
// POST /api/vendedores/[repId]/sin-pedidos  body {days?: number} → envía
//
...
const cleanDays = (raw: unknown): number => {
  ...
  if (!Number.isFinite(n)) return DEFAULT_SIN_PEDIDOS_DAYS;
...
export async function GET(req: Request, { params }: { params: { repId: string } }) {
  ...
  const draft = await buildSinPedidosDigest(supabase, params.repId, days);
