...
import { AlertTriangle, CheckCircle2, HelpCircle, Upload, ShieldAlert } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { formatDate } from "@/lib/utils";
import type { RestockEvaluation } from "@/lib/restock-review";
import { SyncSalesFromDriveButton } from "@/components/restock/SyncSalesFromDriveButton";
...
export function RestockInventoryEvaluation({ rows }: { rows: RestockEvaluation[] }) {
  ...
  return <Card className={alerts ? "border-amber-300" : "border-emerald-200"}><CardContent className="space-y-4 p-6">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 className="font-display text-xl">Evaluación contra inventario y ventas</h2><p className="text-sm text-muted-foreground">Apoyo para decidir antes de aprobar. La recomendación no sustituye la revisión comercial.</p></div><div className="flex flex-wrap gap-2"><SyncSalesFromDriveButton />{needsData ? <Button asChild variant="outline" size="sm"><Link href="/catalogo/importar"><Upload className="mr-1 h-4 w-4" /> Cargar inventario</Link></Button> : null}</div></div>
